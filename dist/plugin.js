...
function preservedChunks(owners, conditions, body, last = []) {
  ...
  const chunks2 = [];
  for (let i = 0; i < body.length; i += ACTION_ROOM) chunks2.push(body.slice(i, i + ACTION_ROOM));
  if (chunks2.length === 0 || chunks2[chunks2.length - 1].length + last.length > ACTION_ROOM) chunks2.push([]);
  chunks2[chunks2.length - 1].push(...last);
  return chunks2.map((actions) => trigger(owners, conditions, [...actions, "Preserve Trigger()"]));
}
function chunks(owners, conditions, body) {
  const out = [];
...
var OWNED = (name, description, required = false) => ({ name, description, required, owned: true });
function countedUnit(text) {
...
var KINDS = [
  ...
  },
  {
    spec: {
      ...
      params: [OWNED("unit", "what a player must keep to stay in (default Any unit)"), P2("players", "humans (default) or player numbers"), P2("grace", "seconds before elimination can happen, so a slow start is not a loss (default 10)")],
      ends: [{ result: "both" }]
    },
    build(r, ctx) {
  ...
  },
  {
    spec: {
      ...
      params: [OWNED("unit", "the unit that must survive", true), P2("players", "humans (default) or player numbers"), P2("grace", "seconds before it can happen (default 5)"), P2("message", "text shown to everyone when it happens (default none)")],
      ends: [{ result: "loss" }]
    },
    build(r) {
      const unit = r.str("unit");
  ...
  },
  {
    ...
    build(r, _ctx, dc) {
      ...
      for (let k = 1; k <= waves; k++) {
        ...
        for (const spawn of spawns) actions.push(a.create(enemy, unit, n2, spawn), a.order(enemy, "Any unit", spawn, goal, "attack"));
        if (announce) actions.unshift(a.text(`Wave ${k}: ${n2} ${unit}${spawns.length > 1 ? ` in each of ${spawns.length} lanes` : ""}`));
  ...
  },
  {
    spec: {
      ...
      params: [L("location", "the shop's beacon location", true), P2("unit", "what is sold", true), P2("price", "minerals (default 100)"), P2("gas", "gas (default 0)"), OWNED("buyer", "which unit must stand on the beacon (default Any unit)"), L("deliver", "where the bought unit appears (default the shop location)"), P2("players", "humans (default) or player numbers")]
    },
    build(r) {
...
function matchesSearch(m, query) {
  ...
  if (!q2) return true;
  return [m.name, m.head.meta.scenarioName, m.head.meta.tileset, m.head.fileName, m.head.note].some((s) => !!s && s.toLowerCase().includes(q2));
}
function listSkeleton(ctx, rows) {
  const w = ctx.api.ui.widgets;
  return Array.from({ length: rows }, () => h2(
    "div",
    { className: "sd-map sd-ghost" },
    w.skeleton({ block: true, width: "56px", height: 56 }),
    h2("div", null, w.skeleton({ lines: 3 }))
  ));
}
function detailSkeleton(ctx, m) {
  const w = ctx.api.ui.widgets;
  return h2(
    "div",
    { className: "sd-detail" },
    h2(
      ...
      h2(
        ...
        m ? h2("div", { className: "sd-title" }, m.name) : w.skeleton({ width: "60%", height: 16 }),
        w.skeleton({ lines: 2 })
      )
    ),
...
function openMapsDialog(ctx, link) {
  ...
  return api.ui.dialog({
    ...
    mount(body, dialog) {
      ...
      const covered = async (label, work) => {
      ...
      const renderList = () => {
        ...
        for (const m of shown) {
          ...
          listBox.append(h2(
            "div",
            { className: `sd-map${pickedId === m.id ? " sd-picked" : ""}`, "data-id": m.id, onClick: () => void pick(m.id) },
            thumb(m.head.meta),
            h2(
              "div",
              { className: "sd-map-text" },
              h2(
                "div",
                { className: "sd-name-row" },
                h2("span", { className: "sd-name", title: m.name }, m.name),
                h2("span", { className: "sd-when", title: formatDate(m.updatedAt) }, ago(m.updatedAt))
              ),
              h2("div", { className: "sd-sub" }, describeMeta(m.head.meta) || m.head.fileName),
              h2("div", { className: "sd-sub" }, sizeLine(m)),
              kept ? h2("div", { className: "sd-badge" }, sharedLine(kept)) : null
            )
          ));
        }
      };
      listBox.addEventListener("keydown", (e) => {
        ...
        if (e.key === "ArrowDown" || e.key === "ArrowUp") {
          ...
          e.preventDefault();
          detailBox.querySelector(".sd-open")?.click();
        }
      });
      ...
      const renderDetail = () => {
        ...
        for (const r of m.history) {
          ...
          const row = h2(
            "div",
            { className: `sd-rev${on ? " sd-picked" : ""}`, onClick: () => {
              if (!on) {
                pickedRevision = r;
                renderDetail();
              }
      ...
      root2.append(
        h2("div", { className: "sd-toolbar" }, search, storageBox, refresh),
        split,
        h2("div", { className: "sd-btns" }, saveHere, h2("div", { className: "sd-grow" }, status))
      );
      void load();
...
var SharedMap = class _SharedMap {
  ...
  lastHeard = 0;
  online = () => {
    if (this.phase === "reconnecting") this.retryNow();
  };
  listeners = /* @__PURE__ */ new Set();
...
function newLines(ours, theirs) {
  const last = ours.at(-1);
  if (!last) return theirs;
  const same = (l) => l.from === last.from && l.at === last.at && l.text === last.text;
