  ...
  return [
    {
      ...
      run: (input, { api }) => {
        ...
        const section = str(input.section) as ReferenceSection;
        if (section && !REFERENCE_SECTIONS.includes(section)) return fail(`section must be one of ${REFERENCE_SECTIONS.join(", ")}.`);
        const text = referenceDetailFor(api, part, { query: str(input.query), section: section || undefined });
        return text === undefined ? fail("No map is open.") : capResult(text, 80_000);
      },
    },
    {
    ...
    },
    {
      ...
      run: (input, { api }) => {
        ...
        const p = paged(out, input, 300, 2000);
    ...
    },
    {
      ...
      run: (input, { api }) => {
        ...
        const p = paged(out, input, 200, 2000);
